package com.otilm.core.service.cmp.mock;
...
public class CertTestUtil {
    ...
    public static X509Certificate createCertificateV3(X500Name subject, SubjectPublicKeyInfo publicKey,
            X509Certificate issuingCert, Extensions extensionsFromTemplate) throws NoSuchAlgorithmException,
            CertificateException, OperatorCreationException, CertIOException, PEMException {
        return new CertificationGeneratorStrategy()
                .generateCertificate(subject, publicKey, issuingCert, getPrivateKeyForSigning(),
                        extensionsFromTemplate);
    }

    private static class CertificationGeneratorStrategy {
        public X509Certificate generateCertificateCA(KeyPair issuerKeyPair, KeyPair subjectKeyPair, X500Name issuer,
                X500Name subject) throws OperatorCreationException, CertificateException, NoSuchAlgorithmException,
            ...
            X509v3CertificateBuilder certificateBuilder = new JcaX509v3CertificateBuilder(/* issuer */issuer,
                    /* serialNumber */new BigInteger(10, new SecureRandom()), /* start */new Date(),
                    /* until */Date
                            .from(LocalDate
                                    .now()
                                    .plus(365 * 10, ChronoUnit.DAYS)
                                    .atStartOfDay()
                                    .toInstant(ZoneOffset.UTC)),
                    /* subject */subject, /* publicKey */pubKeyInfo);
            // Basic Constraints
            PublicKey pubKey = JCA_KEY_CONVERTER.getPublicKey(pubKeyInfo);
            final JcaX509ExtensionUtils extUtils = new JcaX509ExtensionUtils();
            // *
            certificateBuilder
                    .addExtension(new ASN1ObjectIdentifier("2.5.29.15"), false,
                            new X509KeyUsage(X509KeyUsage.digitalSignature | X509KeyUsage.nonRepudiation
                                    | X509KeyUsage.keyEncipherment | X509KeyUsage.cRLSign
                                    | X509KeyUsage.dataEncipherment));
            // */
            certificateBuilder
                    .addExtension(Extension.subjectKeyIdentifier, false, extUtils.createSubjectKeyIdentifier(pubKey));
            certificateBuilder.addExtension(Extension.basicConstraints, true, new BasicConstraints(true));// true is for
                                                                                                          // CA
            // -------------------------------------
        ...
        public X509Certificate generateCertificate(X500Name subject, SubjectPublicKeyInfo publicKey,
                X509Certificate issuer, PrivateKey issuerPrivateKey, Extensions extensionsFromTemplate)
                throws CertificateException, OperatorCreationException, CertIOException, PEMException,
            ...
            X509v3CertificateBuilder certificateBuilder = new JcaX509v3CertificateBuilder(
                    ...
                    /* subject */new X500Principal(subject.toString()), /* publicKey */pubKey);
            final JcaX509ExtensionUtils extUtils = new JcaX509ExtensionUtils();
        ...
        public static Certificate selfSign(KeyPair keyPair, String subjectDN)
            ...
            String signatureAlgorithm = "SHA256WithRSA"; // <-- Use appropriate signature algorithm based on your
                                                         // keyPair algorithm.
