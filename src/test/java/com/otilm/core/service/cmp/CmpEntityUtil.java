...
public class CmpEntityUtil {
    ...
    public static Certificate createCertificate(BigInteger serialNumber, CertificateState state,
    ...
    public static Certificate createCertificate(CertificateState state, CertificateContent certificateContent,
    ...
    public static Certificate createCertificate(X509CertificateHolder x509certificate, CertificateState state,
            CertificateContent certificateContent) throws CMPException, OperatorCreationException {
        DigestCalculator digester = CmpTestUtil.createMessageDigest(x509certificate);
        Certificate certificate = createCertificate(x509certificate.getSerialNumber(), state, certificateContent);
        certificate.setFingerprint(new DEROctetString(digester.getDigest()).toString().substring(1)/* remove ''#' */);
        return certificate;
    }

    public static Certificate createCertificate(DigestCalculator digester, BigInteger serialNumber,
            CertificateState state, CertificateContent certificateContent) {
        Certificate certificate = createCertificate(serialNumber, state, certificateContent);
        certificate.setFingerprint(new DEROctetString(digester.getDigest()).toString().substring(1)/* remove ''#' */);
        return certificate;
    }
    ...
    public static Certificate createCertificate(BigInteger serialNumber, CertificateState state,
            CertificateContent certificateContent, CryptographicKey key) {
        Certificate certificate = createCertificate(serialNumber, state, certificateContent);
    ...
    public static Certificate createCertificate(BigInteger serialNumber, CertificateState state,
            CertificateContent certificateContent, UUID issuerCertificateUuid, CryptographicKey key) {
        Certificate certificate = createCertificate(serialNumber, state, certificateContent);
        certificate.setKey(key);
    ...
    public static CmpTransaction createTransaction(String transactionId, Certificate issuedCertificate,
            CmpProfile cmpProfile, CmpTransactionState state) {
    ...
    public static CryptographicKeyItem createCryptographicKeyItem(CryptographicKey key, UUID keyReferenceUuid,
            KeyType keyType, KeyAlgorithm keyAlgorithm, String fingerprint) {
