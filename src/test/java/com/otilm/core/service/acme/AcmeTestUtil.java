...
public final class AcmeTestUtil {
     ...
     *
     * <p>
     * When {@code accountId} is {@code null} the public key is embedded in the JWK header (used for new-account
     * requests). Otherwise, a {@code kid} header pointing to the account URL is used.
     *
     * <p>
     * A fresh nonce is persisted to {@code acmeNonceRepository} so the service can validate it.
     *
     ...
     */
    public static String createJwsRequest(ObjectMapper objectMapper, KeyPair acmeKeyPair,
            AcmeNonceRepository acmeNonceRepository, Object payload, String url, String accountId,
        ...
        Payload jwsPayload = payload != null ? new Payload(objectMapper.writeValueAsString(payload)) : new Payload("");
    ...
    public static X509Certificate createTestCertificate(KeyPair keyPair, String commonName) throws Exception {
        ...
        JcaX509v3CertificateBuilder certBuilder = new JcaX509v3CertificateBuilder(name, serial, notBefore, notAfter,
