 ...
 * <p>
 * Core defines almost no DTOs of its own — the contract layer is the published {@code interfaces} artifact, consumed
 * by roughly forty connector repositories that will still be on Spring Boot 3.5 when core moves to 4.1. So the risk
 * worth testing is not "does some DTO serialize", but "do the mapper-level rules that shape <i>every</i> DTO still
 * hold": null omission, date rendering, and the polymorphic attribute types embedded in nearly every response.
 * <p>
 * Rather than pin a few enormous DTOs field by field — brittle, and no more informative — these tests pin the
 * rules themselves against representative carriers. {@code ResponseAttribute} in particular appears in almost every
 * detail response in the platform, so its shape is the contract's most widely shared component.
 */
class ApiContractGoldenTest {
    ...
    /**
     * Dates are the other global rule. With {@code WRITE_DATES_AS_TIMESTAMPS} disabled they render as ISO-8601
     * strings; were that lost in the upgrade, every date in every response would become a number, and every consumer
     * parsing them as strings would break simultaneously.
     */
    @Test
    ...
    /**
     * {@code ResponseAttribute} discriminates on {@code version}, and both arms appear in live responses because the
     * v2 and v3 attribute schemas coexist. Pinning both proves a v2 attribute cannot start serializing as a v3 one
     * (or vice versa) — a confusion the FE would render as an attribute with no content.
     */
    @Test
    void responseAttributeV3KeepsItsVersionDiscriminatorAndContentShape() {
        ...
        GoldenJson.assertMatchesGoldenAndRoundTrips(
                "api-response-attribute-v3", mapper, attribute, ResponseAttribute.class);

        assertThat(mapper.valueToTree(attribute).path("version").asText())
                .isEqualTo(AttributeVersion.V3.getCode());
    }
    ...
    void responseAttributeV2KeepsItsVersionDiscriminatorAndContentShape() {
        ...
        GoldenJson.assertMatchesGoldenAndRoundTrips(
                "api-response-attribute-v2", mapper, attribute, ResponseAttribute.class);

        assertThat(mapper.valueToTree(attribute).path("version").asText())
                .isEqualTo(AttributeVersion.V2.getCode());
    }
    ...
    void uuidResponseKeepsItsShape() {
        GoldenJson.assertMatchesGoldenAndRoundTrips(
                "api-uuid-response", mapper, new UuidDto("d41f8c9b-0000-4000-8000-000000000005"), UuidDto.class);
    }
}
