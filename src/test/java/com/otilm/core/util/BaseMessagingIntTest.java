 ...
 *
 * <p>
 * {@code @DirtiesContext(BEFORE_CLASS)} is intentional and must stay here. The static {@link RabbitMQContainer} is
 * started/stopped per concrete subclass by the Testcontainers JUnit 5 extension, so each subclass gets a new container
 * at a new random host port. Spring's context cache key does NOT include {@link DynamicPropertySource} values.
 * </p>
 */
@SpringBootTest
...
public abstract class BaseMessagingIntTest extends BaseSpringBootTest {
    ...
    static void rabbitMQProperties(DynamicPropertyRegistry registry) throws IOException, InterruptedException {
        ...
        registry
                .add("spring.messaging.broker-url", () -> String
                        .format("amqp://%s:%d", rabbitMQContainer.getHost(), rabbitMQContainer.getAmqpPort()));
    ...
    public void setupAuth() {
        // Skip clearTables() for messaging tests - they use different database schema
