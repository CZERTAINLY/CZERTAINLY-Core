...
/**
 * WireMock stub helpers for the v3 authority provider wire contract. Each static method registers stubs for a specific
 * v3 connector endpoint using paths from the {@code CertificateApiClient} v3 constants.
 */
public final class V3ConnectorStubs {
     ...
     *
     * @param metaJson JSON array of MetadataAttribute objects; pass {@code null} or {@code "[]"} for a meta-free
     * response
     */
    public static void stubRegisterSync(WireMockServer wm, String metaJson) {
        ...
        wm
                .stubFor(post(urlEqualTo(REGISTER_PATH))
                        .willReturn(
     ...
     *
     * @param trackingMeta JSON array of MetadataAttribute tracking objects; pass {@code null} or {@code "[]"} for empty
     * meta
     */
    ...
    /**
     * Stubs the connector's register-operation attribute-list endpoint to return an empty schema, so the unconditional
     * {@code mergeAndValidateRegisterAttributes} on the register-capable path succeeds without real register-attribute
     * data. Every register-capable stub needs this — a real register-capable connector exposes
    ...
    public static void stubRegisterStatus(WireMockServer wm, String scenarioName) {
        wm
                .stubFor(post(urlEqualTo(REGISTER_STATUS))
    ...
    public static void stubV3IssueSync(WireMockServer wm, String certData) {
        wm
                .stubFor(post(urlEqualTo(ISSUE_PATH))
                        .willReturn(
                                aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody("""
    ...
    public static void stubAttributesAndValidate(WireMockServer wm) {
        ...
        wm
                .stubFor(post(urlMatching("/v3/authorityProvider/certificates/.*/attributes"))
                        .willReturn(WireMock.okJson("[]")));
        wm
                .stubFor(get(urlMatching("/v3/authorityProvider/certificates/.*/attributes"))
                        .willReturn(WireMock.okJson("[]")));
        // v3 authority attributes
        wm.stubFor(get(urlMatching("/v3/authorityProvider/attributes")).willReturn(WireMock.okJson("[]")));
        wm.stubFor(post(urlMatching("/v3/authorityProvider/attributes/validate")).willReturn(WireMock.okJson("true")));
        // v3 RA profile attributes
        wm.stubFor(get(urlMatching("/v3/authorityProvider/raProfile/attributes")).willReturn(WireMock.okJson("[]")));
        wm
                .stubFor(post(urlMatching("/v3/authorityProvider/raProfile/attributes/validate"))
