...
public class CustomNameStyleTest {
    ...
    @Test
    public void testCustomNameStyle() {
        X500Principal x500Principal = new X500Principal("CN=Demo Client Sub CA,O=3Key Company s.r.o.");
        logger.info(X500Name.getInstance(X500NameStyleCustom.INSTANCE, x500Principal.getEncoded()).toString());
        x500Principal = new X500Principal("CN=Demo Client Sub CA,EMAIL=3key@company.com");
        logger.info(X500Name.getInstance(X500NameStyleCustom.INSTANCE, x500Principal.getEncoded()).toString());
        logger.info(X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal("CN=Certificate Authority; O=Organization;L=Location, ST=State, C=US").getEncoded()).toString());
        String string = X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal("CN=Certificate Authority;O=Organization;L=Location,ST=State, C=US").getEncoded()).toString();
        logger.info(string);
        String stringMod = X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal("CN=Certificate Authority; L=Location,ST=State, C=US;O=Organization").getEncoded()).toString();
        logger.info(stringMod);
        String stringMod2 = X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal("CN=Certificate Authority; O=Organization; L=Location,ST=State; C=US").getEncoded()).toString();
        logger.info(stringMod2);
        logger.info(X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal("CN=Example Root CA; O=Example Corp; EMAIL=admin@example.com,C=US").getEncoded()).toString());
...
//        new X500Principal("E=3key@company.com, CN=Demo Client Sub CA");
    ...
    private String getDnWithCustomStyle(String dn){
        return X500Name.getInstance(X500NameStyleCustom.INSTANCE, new X500Principal(dn).getEncoded()).toString();
    }
    }
