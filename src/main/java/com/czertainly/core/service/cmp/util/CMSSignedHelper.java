...
public class CMSSignedHelper {
    ...
    Store getCertificates(ASN1Set certSet)
    {
        if (certSet != null)
        {
            ...
            {
                org.bouncycastle.asn1.ASN1Primitive obj = ((ASN1Encodable)en.nextElement()).toASN1Primitive();
    ...
    Store getCRLs(ASN1Set crlSet)
    {
        if (crlSet != null)
        {
            ...
            for (Enumeration en = crlSet.getObjects(); en.hasMoreElements();)
            {
                ...
                if (obj instanceof ASN1Sequence)
                {
                    crlList.add(new X509CRLHolder(org.bouncycastle.asn1.x509.CertificateList.getInstance(obj)));
                }
            }
