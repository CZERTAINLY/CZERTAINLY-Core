 ...
 * <p>
 * Apache HttpClient's own defaults are unsuitable for a client held for the lifetime of the process,
 * and {@link ClientHttpRequestFactoryBuilder#detect()} passes them straight through, so every consumer
 * builds from here instead of from the plain builder.
 */
public final class PlatformHttpClients {

    /**
     * Upper bound on connections a single client may hold, per endpoint and in total. Apache's defaults
     * of 5 per route and 25 overall throttle callers well below what the server itself allows — it runs
     * on virtual threads, so request concurrency is not otherwise capped.
     */
     ...
     * <p>
     * Cookies are off because one jar would be shared by every caller in the process, replaying one
     * user's session cookie onto everyone else's requests. Status-code retries are off because Apache
     * honours a {@code Retry-After} header verbatim, which lets a degraded peer decide how long our
     * threads block; the retry after an I/O error is kept, as that is what recovers a pooled connection
     * the peer closed concurrently.
     * <p>
     ...
     * peer returned — the response-driven branch is what honours {@code Retry-After}.
     */
