...
public class CertificateUtil {
    ...
    public static X509Certificate getX509Certificate(byte[] decoded) throws CertificateException {
        try {
            X509Certificate certificate = (X509Certificate) new CertificateFactory()
                    .engineGenerateCertificate(new ByteArrayInputStream(decoded));
            if (certificate.getPublicKey() == null) {
                if (logger.isDebugEnabled()) {
                    logger
                            .debug("Certificate has no public key, trying to generate certificate with BouncyCastleProvider: {}",
                                    Base64.getEncoder().encodeToString(decoded));
                }
                java.security.cert.CertificateFactory certificateFactory = java.security.cert.CertificateFactory
            ...
            throw new CertificateException(
                    "Failed to parse and create X509Certificate object from provided certificate content");
        }
    }
    ...
    public static CertificateSubjectType getCertificateSubjectType(X509Certificate certificate,
            boolean subjectDnEqualsIssuerDn) {
    ...
    public static Map<String, List<String>> getSAN(CertificateRequest certificateRequest) {
        ...
        if (certificateRequest instanceof Pkcs10CertificateRequest request) {
            ...
            Extensions extensions = Extensions
                    .getInstance(request.getCertificateRequestMessage().getCertTemplate().getExtensions());
            gns = GeneralNames.fromExtensions(extensions, Extension.subjectAlternativeName);
        }
        ...
        for (GeneralName name : names) {
            ...
            try {
                ...
                sanTypeName = SAN_TYPE_MAP.get(sanType);
                String value = getSanValueString(sanType,
                        sanType == GeneralName.otherName || sanType == GeneralName.ediPartyName
                                ? name
                                : name.getName());
                ((ArrayList<String>) sans.get(sanTypeName)).add(value);
            } catch (Exception e) {
    ...
    private static String getSanValueString(Integer sanType, Object value) {
        ...
        if (sanType == GeneralName.ediPartyName) {
            ...
            return ediPartySeq.size() == 1
                    ? "Party=" + ediPartySeq.getObjectAt(0).toString()
                    : "Assigner=%s, Party=%s"
                            .formatted(ediPartySeq.getObjectAt(0).toString(), ediPartySeq.getObjectAt(1).toString());
        }
    ...
    private static Map<String, List<String>> buildEmptySans() {
        return SAN_TYPE_MAP
                .values()
                .stream()
                .collect(Collectors.toMap(sanTypeName -> sanTypeName, sanTypeName -> new ArrayList<>(), (a, b) -> b));
    }
    ...
    public static X509Certificate parseUploadedCertificateContent(String certificateContent) {
        ...
        try (StringReader contentReader = new StringReader(new String(decodedContent));
            ...
            // alternative is to read straight with pemParser.readObject() and let to create object encoded in PEM by
            // BouncyCastle
    ...
    public static String getThumbprint(byte[] encodedContent) throws NoSuchAlgorithmException {
    ...
    public static String getSha1Thumbprint(byte[] encodedContent) throws NoSuchAlgorithmException {
    ...
    public static DiscoveryCertificate prepareDiscoveryCertificate(Certificate entry, X509Certificate certificate) {
        ...
        if (entry != null) {
            ...
            setSubjectDNParams(certificateModal, X500Name
                    .getInstance(new PlatformX500NameStyle(false), certificate.getSubjectX500Principal().getEncoded()));
            setIssuerDNParams(certificateModal, X500Name
                    .getInstance(new PlatformX500NameStyle(false), certificate.getIssuerX500Principal().getEncoded()));
    ...
    /**
     * Sets certificate data fields only; does NOT advance state. Callers routing PENDING_ISSUE-&gt;ISSUED through
     * {@link com.otilm.core.service.handler.authority.lifecycle.CertificateStateMachine} call this before the SM
     * transition; callers creating new certificate rows that bypass the SM use {@link #prepareIssuedCertificate}
     * instead.
     */
    public static void stampIssuedFields(Certificate modal, X509Certificate certificate) {
        ...
        if (certificate.getPublicKey() == null) {
            throw new ValidationException(ValidationError.create("Invalid Certificate. Public Key is missing"));
        }
        try {
            modal
                    .setPublicKeyFingerprint(getThumbprint(Base64
                            .getEncoder()
        ...
        if (alternativeSignatureAlgorithm != null && alternativeSignature != null) {
            try {
                modal
                        .setAltSignatureAlgorithm(getAlternativeSignatureAlgorithm(alternativeSignatureAlgorithm)
                                .replace("WITH", "with"));
            } catch (IOException e) {
        ...
        modal
                .setIssuerDnNormalized(
                        X500Name.getInstance(new PlatformX500NameStyle(true), issuerDnPrincipalEncoded).toString());
        modal
                .setSubjectDnNormalized(
                        X500Name.getInstance(new PlatformX500NameStyle(true), subjectDnPrincipalEncoded).toString());
        CertificateSubjectType subjectType = CertificateUtil
                .getCertificateSubjectType(certificate,
                        modal.getSubjectDnNormalized().equals(modal.getIssuerDnNormalized()));
        if (subjectType == CertificateSubjectType.ROOT_CA
        ...
        try {
            ...
            logger
                    .warn("Unable to get the extended key usage for certificate with serial number {} and subject DN {}: {}",
                            modal.getSerialNumber(), modal.getSubjectDn(), e.getMessage());
        }
        if (extendedKeyUsage != null) {
            modal.setExtendedKeyUsage(MetaDefinitions.serializeArrayString(extendedKeyUsage));
            modal
                    .setExtendedKeyUsageCritical(certificate.getCriticalExtensionOIDs() != null
        ...
        try {
            ...
            logger
                    .warn("Unable to parse QCStatements extension for certificate with serial number {} and subject DN {}: {}",
                            modal.getSerialNumber(), modal.getSubjectDn(), e.getMessage());
        }
        if (qc != null) {
            ...
            modal
                    .setQcType(qc.qcType() != null
                            ? MetaDefinitions.serializeArrayString(qc.qcType().stream().map(QcType::name).toList())
                            : null);
            modal
                    .setQcCcLegislation(qc.qcCcLegislation() != null
    ...
    /**
     * Parses the QCStatements extension (OID 1.3.6.1.5.5.7.1.3) of an X.509 certificate and extracts the four
     * operationally relevant statements per ETSI EN 319 412-5. Returns null when the extension is absent.
     */
    private static QcStatementParseResult parseQcStatements(X509Certificate cert) {
        ...
        return new QcStatementParseResult(qcCompliance, qcSscd, qcType.isEmpty() ? null : qcType,
                qcCcLegislation.isEmpty() ? null : qcCcLegislation);
    }

    record QcStatementParseResult(boolean qcCompliance, boolean qcSscd, List<QcType> qcType,
    ...
    public static PublicKey getAltPublicKey(byte[] altPublicKeyInfoEncoded)
    ...
    public static void prepareCertificateFromCsr(Certificate modal, CertificateRequest certificateRequest)
        ...
        try {
            modal
                    .setPublicKeyFingerprint(getThumbprint(Base64
                            .getEncoder()
                            .encodeToString(certificateRequest.getPublicKey().getEncoded())
                            .getBytes(StandardCharsets.UTF_8)));
        } catch (NoSuchAlgorithmException e) {
    ...
    public static void prepareCertificateRequestEntityFromCsr(CertificateRequestEntity modal,
            CertificateRequest certificateRequest) throws NoSuchAlgorithmException, CertificateRequestException {
        CsrIdentity identity = deriveCsrIdentity(certificateRequest);
        modal.setSubjectDn(identity.subjectDn());
        modal.setCommonName(identity.commonName());
        modal.setPublicKeyAlgorithm(identity.publicKeyAlgorithm());
    ...
    /**
     * Identity fields shared by the certificate and certificate-request rows, derived once from a parsed request.
     * {@code Certificate} and {@code CertificateRequestEntity} have no common supertype, so both prepare methods apply
     * these values through their own setters.
     */
    private record CsrIdentity(String subjectDn, String commonName, String publicKeyAlgorithm, String serializedSans) {
    }

    private static CsrIdentity deriveCsrIdentity(CertificateRequest certificateRequest)
            throws NoSuchAlgorithmException, CertificateRequestException {
        if (certificateRequest.getPublicKey() == null) {
            throw new ValidationException(ValidationError.create("Invalid public key in certificate request"));
        }
        ...
        return new CsrIdentity(subjectDN != null ? subjectDN.toString() : null,
    ...
    /**
     * Records the subject identity of a no-CSR registration placeholder from the operator-supplied subject DN. Sets the
     * subject DN, common name and normalized subject; key, SAN and fingerprint fields stay empty until issuance fills
     * them. A blank DN stores only the normalized empty subject (subject carried entirely in the SAN, permitted by RFC
     * 5280 §4.1.2.6).
     */
    public static void applyRegistrationSubject(Certificate modal, String subjectDn) {
        ...
        try {
            ...
            throw new ValidationException(
                    ValidationError.create("Invalid subject DN '%s': %s".formatted(subjectDn, e.getMessage())));
        }
        try {
        ...
        for (GeneralNameEntry entry : subjectAltNames) {
            ...
            if (entry.getType() == null) {
                throw new ValidationException(ValidationError
                        .create("A projected subject alternative name entry carries no type; check the SAN field mappings of the issuance attributes"));
            }
            if (entry.getType() == GeneralNameType.OTHER_NAME
                    && (entry.getOtherNameOid() == null || entry.getOtherNameOid().isBlank())) {
                throw new ValidationException(ValidationError
                        .create("A projected otherName subject alternative name entry carries no OID; check the SAN field mappings of the issuance attributes"));
            }
    ...
    /**
     * Single source of the persisted otherName SAN form, shared by issued-certificate parsing and registration
    ...
    /**
     * Extracts the common name from the subject DN. With multiple CN RDNs the last one in {@link X500Name#getRDNs()}
     * order wins — the non-normalized rendering reverses the RDN array, so this is the leading CN of the
     * {@code subjectDn} string persisted alongside it.
     */
    ...
    public static List<JcaX509CertificateHolder> convertToX509CertificateHolder(List<X509Certificate> certificateChain)
    ...
    public static boolean isValidationEnabled(Certificate certificate,
        ...
        if (Boolean.FALSE.equals(raValidationEnabled)) {
            ...
            if (validationResultDto != null) {
                validationResultDto
                        .setMessage("Validation of certificates in RA Profile %s is disabled."
