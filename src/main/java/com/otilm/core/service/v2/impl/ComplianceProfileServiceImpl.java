...
import com.otilm.core.dao.entity.ComplianceProfile_;
...
public class ComplianceProfileServiceImpl
    ...
    public List<ComplianceProfileListDto> listComplianceProfiles(SecurityFilter filter) {
        return complianceProfileRepository
                ...
                .stream()
                .map(ComplianceProfile::mapToListDto)
                .toList();
    }
    ...
    public ComplianceProfileDto createComplianceProfile(ComplianceProfileRequestDto request)
        ...
        ComplianceProfileDto complianceProfileDto = ruleHandler
                .updateComplianceProfileRules(complianceProfile, request);
        complianceProfileDto
                .setCustomAttributes(attributeEngine
                        .updateObjectCustomAttributesContent(Resource.COMPLIANCE_PROFILE, complianceProfile.getUuid(),
    ...
    public List<BulkActionMessageDto> bulkDeleteComplianceProfiles(List<SecuredUUID> uuids) {
        ...
        for (SecuredUUID uuid : uuids) {
            ...
            try {
                complianceProfile = complianceProfileRepository
                        .findWithAssociationsByUuid(uuid.getValue())
                        .orElseThrow(() -> new NotFoundException(ComplianceProfile.class, uuid));
                deleteComplianceProfile(complianceProfile, false);
            } catch (Exception e) {
                ...
                messages
                        .add(BulkActionMessageDto
                                .failure(uuid.toString(), complianceProfile != null ? complianceProfile.getName() : "",
                                        e, "Delete failed"));
            }
        }
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.LIST)
    public List<ComplianceRuleListDto> getComplianceRules(UUID connectorUuid, String kind, Resource resource,
            String type, String format) throws NotFoundException, ConnectorException {
        if (resource != null && !resource.complianceSubject()) {
            throw new ValidationException(
                    "Cannot list compliance rules for resource %s. Resource does not support compliance check"
        ...
        if (connectorUuid == null) {
            List<ComplianceInternalRule> internalRules = resource != null
                    ? internalRuleRepository.findByResource(resource)
                    : internalRuleRepository.findAll();
        ...
        FunctionGroupCode complianceFunctionGroup = ComplianceProfileRuleHandler
                .validateComplianceProvider(connectorDto, kind);
        if (complianceFunctionGroup == FunctionGroupCode.COMPLIANCE_PROVIDER_V2) {
            List<ComplianceRuleResponseDto> providerRules = resource == null
                    ? connectorApiFactory
                            .getComplianceApiClientV2(connectorDto)
                            .getComplianceRules(connectorDto, kind, null, null, null)
                    : connectorApiFactory
                            .getComplianceApiClientV2(connectorDto)
                            .getComplianceRules(connectorDto, kind, resource, type, format);
            ...
            List<ComplianceRulesResponseDto> providerRules = connectorApiFactory
                    .getComplianceApiClient(connectorDto)
                    .getComplianceRules(connectorDto, kind, type == null ? null : List.of(type));
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.LIST)
    public List<ComplianceGroupListDto> getComplianceGroups(UUID connectorUuid, String kind, Resource resource)
        ...
        FunctionGroupCode complianceFunctionGroup = ComplianceProfileRuleHandler
                .validateComplianceProvider(connectorDto, kind);
        if (complianceFunctionGroup == FunctionGroupCode.COMPLIANCE_PROVIDER_V2) {
            List<ComplianceGroupResponseDto> providerGroups = connectorApiFactory
                    .getComplianceApiClientV2(connectorDto)
                    .getComplianceGroups(connectorDto, kind, resource);
            ...
            List<ComplianceGroupsResponseDto> providerGroups = connectorApiFactory
                    .getComplianceApiClient(connectorDto)
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.LIST)
    public List<ComplianceRuleListDto> getComplianceGroupRules(UUID groupUuid, UUID connectorUuid, String kind)
            throws NotFoundException, ConnectorException {
        Connector connector = connectorRepository
                .findByUuid(connectorUuid)
                .orElseThrow(() -> new NotFoundException(Connector.class, connectorUuid));
        ...
        if (complianceFunctionGroup == FunctionGroupCode.COMPLIANCE_PROVIDER_V2) {
            return connectorApiFactory
                    .getComplianceApiClientV2(connectorDto)
                    .getComplianceGroupRules(connectorDto, kind, groupUuid)
                    .stream()
                    .map(providerRule -> {
                        ComplianceRuleListDto dto = new ComplianceRuleListDto();
                        dto.setUuid(providerRule.getUuid());
        ...
        return connectorApiFactory
                .getComplianceApiClient(connectorDto)
                .getComplianceGroupRules(connectorDto, kind, groupUuid.toString())
                .stream()
                .map(providerRule -> {
                    ComplianceRuleListDto dto = new ComplianceRuleListDto();
                    dto.setUuid(UUID.fromString(providerRule.getUuid()));
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.UPDATE)
    public ComplianceRuleListDto createComplianceInternalRule(ComplianceInternalRuleRequestDto request)
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.UPDATE)
    public ComplianceRuleListDto updateComplianceInternalRule(UUID internalRuleUuid,
            ComplianceInternalRuleRequestDto request) throws NotFoundException {
        ComplianceInternalRule internalRule = internalRuleRepository
                .findByUuid(internalRuleUuid)
    ...
    public void deleteComplianceInternalRule(UUID internalRuleUuid) throws NotFoundException {
        ...
        Long associatedProfilesCount = complianceProfileRepository
                .countByComplianceRulesInternalRuleUuid(internalRuleUuid);
        if (associatedProfilesCount > 0) {
            List<String> profileNames = complianceProfileRepository
                    .findNamesByComplianceRulesInternalRuleUuid(internalRuleUuid);
            throw new ValidationException(
                    "Cannot delete the compliance internal rule as it is associated to compliance profiles: %s"
                            .formatted(String.join(", ", profileNames)));
        }
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.UPDATE)
    public void patchComplianceProfileRules(SecuredUUID uuid, ComplianceProfileRulesPatchRequestDto request)
            throws NotFoundException, ConnectorException {
        ...
        if (request.getConnectorUuid() == null) {
            // internal rule
            if (request.isRemoval() && !complianceProfileRuleRepository
                    .existsByComplianceProfileUuidAndInternalRuleUuid(uuid.getValue(), request.getRuleUuid())) {
                throw new NotFoundException("Compliance profile does not have assigned internal rule with UUID %s"
                        .formatted(request.getRuleUuid()));
            }

            complianceProfileRuleRepository
                    .deleteByComplianceProfileUuidAndInternalRuleUuid(uuid.getValue(), request.getRuleUuid());
            ...
            // provider rule
            if (request.isRemoval() && !complianceProfileRuleRepository
                    .existsByComplianceProfileUuidAndConnectorUuidAndKindAndComplianceRuleUuid(uuid.getValue(),
                            request.getConnectorUuid(), request.getKind(), request.getRuleUuid())) {
                throw new NotFoundException(
                        "Compliance profile does not have assigned rule with UUID %s from connector %s and kind %s"
                                .formatted(request.getRuleUuid(), request.getConnectorUuid(), request.getKind()));
            }
            ...
            complianceProfileRuleRepository
                    .deleteByComplianceProfileUuidAndConnectorUuidAndKindAndComplianceRuleUuid(uuid.getValue(),
                            request.getConnectorUuid(), request.getKind(), request.getRuleUuid());
            if (!request.isRemoval()) {
                ComplianceRuleResponseDto providerRule = ruleHandler
                        .getProviderRule(request.getConnectorUuid(), request.getKind(), request.getRuleUuid());
                ruleHandler
                        .createComplianceProfileProviderRuleAssoc(uuid.getValue(), request.getConnectorUuid(),
                                request.getKind(), providerRule, request.getAttributes());
            }
        }
    }
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.UPDATE)
    public void patchComplianceProfileGroups(SecuredUUID uuid, ComplianceProfileGroupsPatchRequestDto request)
            throws ConnectorException, NotFoundException {
        if (!complianceProfileRepository.existsById(uuid.getValue())) {
            throw new NotFoundException(ComplianceProfile.class, uuid.getValue());
        }
        if (request.isRemoval() && !complianceProfileRuleRepository
                .existsByComplianceProfileUuidAndConnectorUuidAndKindAndComplianceGroupUuid(uuid.getValue(),
                        request.getConnectorUuid(), request.getKind(), request.getGroupUuid())) {
            throw new NotFoundException(
                    "Compliance profile does not have assigned group with UUID %s from connector %s and kind %s"
                            .formatted(request.getGroupUuid(), request.getConnectorUuid(), request.getKind()));
        }

        complianceProfileRuleRepository
                .deleteByComplianceProfileUuidAndConnectorUuidAndKindAndComplianceGroupUuid(uuid.getValue(),
                        request.getConnectorUuid(), request.getKind(), request.getGroupUuid());
        if (!request.isRemoval()) {
            ComplianceGroupResponseDto providerGroup = ruleHandler
                    .getProviderGroup(request.getConnectorUuid(), request.getKind(), request.getGroupUuid());
            ruleHandler
                    .createComplianceProfileProviderGroupAssoc(uuid.getValue(), request.getConnectorUuid(),
                            request.getKind(), providerGroup);
        }
    }
    ...
    public List<ResourceObjectDto> getAssociations(SecuredUUID uuid) throws NotFoundException {
        ...
        for (ComplianceProfileAssociation association : complianceProfile.getAssociations()) {
            associatedObjects
                    .add(resourceService.getResourceObject(association.getResource(), association.getObjectUuid()));
        }
    ...
    @Override
    @ExternalAuthorization(resource = Resource.COMPLIANCE_PROFILE, action = ResourceAction.LIST)
    public List<ComplianceProfileListDto> getAssociatedComplianceProfiles(Resource resource,
            UUID associationObjectUuid) {
        List<ComplianceProfileAssociation> associations = complianceProfileAssociationRepository
    ...
    public void associateComplianceProfile(SecuredUUID uuid, Resource resource, UUID associationObjectUuid)
        ...
        if (complianceProfileAssociationRepository
                .existsByComplianceProfileUuidAndResourceAndObjectUuid(uuid.getValue(), resource,
                        associationObjectUuid)) {
            throw new AlreadyExistException("Compliance profile is already associated to %s with UUID %s"
    ...
    public void disassociateComplianceProfile(SecuredUUID uuid, Resource resource, UUID associationObjectUuid)
        ...
        if (!complianceProfileAssociationRepository
                .existsByComplianceProfileUuidAndResourceAndObjectUuid(uuid.getValue(), resource,
                        associationObjectUuid)) {
            throw new NotFoundException("Compliance profile is not associated to %s with UUID %s"
                    .formatted(resource.getLabel(), associationObjectUuid));
        }

        complianceProfileAssociationRepository
                .deleteByComplianceProfileUuidAndResourceAndObjectUuid(uuid.getValue(), resource,
                        associationObjectUuid);
    }
    ...
    private void deleteComplianceProfile(ComplianceProfile complianceProfile, boolean isForce)
        ...
        if (!isForce && !complianceProfile.getAssociations().isEmpty()) {
            ...
            String associatedObjects = String
                    .join(", ",
                            associations
                                    .stream()
                                    .map(a -> "%s '%s'".formatted(a.getResource().getLabel(), a.getName()))
                                    .toList());
            throw new ValidationException(
                    "Unable to delete compliance profile due to associated resource objects: " + associatedObjects);
        }
    ...
    private Set<ConditionItem> createConditionItems(List<ConditionItemRequestDto> conditionItemRequestDtos,
            ComplianceInternalRule internalRule) {
