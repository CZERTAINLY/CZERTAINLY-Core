...
public interface RegisterCapability {
    ...
    List<BaseAttribute> listRegisterAttributes(AuthorityInstanceReference authority, RaProfile raProfile)
            throws ConnectorException;

    /**
     * Sends a pre-registration request; the CA reserves a slot/identity and returns a tracking handle in the response
     * metadata.
     *
     * <p>
     * <b>Failure contract:</b> a failure after the connector accepts (2xx/202) must throw
     ...
     * {@code csrAttributes} or from the flat subjectDn/subjectAltName/extensions — so the persisted placeholder and the
     * wire derive from the same content; when null the adapter builds it from the request's flat fields defensively
    ...
    /**
     * Issues a certificate against a prior registration: the client CSR is forwarded intact, the binding's CA handle is
     * replayed as {@code meta}, and {@code requestContent} carries the registered identity for connectors advertising
     * {@code CERTIFICATE_IDENTITY_OVERRIDE}. Failure contract is the same as {@link #register}.
     *
