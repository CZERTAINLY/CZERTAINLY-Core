...
/**
 * Adapter for v3 authority provider connectors. Wraps {@link CertificateSyncApiClient} (v3) and
 * {@link AuthoritySyncApiClient} (v3) via {@link ConnectorApiFactory}.
 *
 * <p>
 * Issue/renew/register: 200 = SYNC_OK, 202 = ASYNC_ACCEPTED. Revoke: 204 = SYNC_NO_CONTENT, 202 = ASYNC_ACCEPTED, 200 =
 * SYNC_OK (meta only). Cancel: 204 = CANCELLED; 422 with OPERATION_PAST_POINT_OF_NO_RETURN = refused; 422 with
 * REGISTRATION_NOT_FOUND or OPERATION_NOT_TRACKED = not tracked.
 * </p>
 *
 * <p>
 * Post-acceptance failures (local mapping after 200/202) are wrapped in
...
public class AuthorityProviderV3Adapter extends AbstractAuthorityProviderAdapter
    ...
    public AuthorityProviderV3Adapter(ConnectorInternalService connectorService,
            ...
            OperationAttributeResolver operationAttributeResolver, OutboundSecretContainment outboundContainment,
    ...
    public List<BaseAttribute> listAuthorityInstanceAttributes(AuthorityInstanceReference authority)
            throws ConnectorException {
        ApiClientConnectorInfo connectorDto = connectorForApiClient(authority);
        return connectorApiFactory.getAuthorityInstanceApiClientV3(connectorDto).listAuthorityAttributes(connectorDto);
    }
    ...
    public List<BaseAttribute> listRaProfileAttributes(AuthorityInstanceReference authority) throws ConnectorException {
        ...
        List<BaseAttribute> response = connectorApiFactory
                .getAuthorityInstanceApiClientV3(connectorDto)
    ...
    public List<AdapterOperationResult> getCaCertificates(AuthorityInstanceReference authority, RaProfile raProfile)
        ...
        List<CertificateDataResponseDto> certificates = connectorApiFactory
                .getAuthorityInstanceApiClientV3(connectorDto)
                .getCaCertificates(connectorDto, request)
                .getCertificates();
    ...
    @Override
    public List<BaseAttribute> listRegisterAttributes(AuthorityInstanceReference authority, RaProfile raProfile)
            throws ConnectorException {
        ApiClientConnectorInfo connectorDto = connectorForApiClient(authority);
        CertificateAttributeListRequestDtoV3 request = attributeListRequest(authority, raProfile);
        List<BaseAttribute> response = connectorApiFactory
     ...
     * and shows operators. That {@code meta} is connector-controlled, but the connector is the legitimate holder of the
     * resolved secret (an echo is no new disclosure to an untrusted party), and — unlike the read-only list fetch —
     * failing closed after the connector has already committed the operation would diverge Core's state from the
     * connector's. {@code cancel} returns no connector content.
     */
    @SafeVarargs
    ...
    /**
     * Runs a v3 connector call and maps the response, applying the post-acceptance failure rule: once the connector
     * returns 2xx, a failure in the local mapping is surfaced as {@link ConnectorAcceptedButLocalFailureException}
     * (callers must NOT roll back state). Connector-side failures (the call itself throwing) propagate unchanged.
     */
    private AdapterOperationResult executeWithAcceptanceGuard(CertificateOperation op, ConnectorCall call,
    ...
    private List<RequestAttribute> authorityAttributesFor(AuthorityInstanceReference authority)
            throws ConnectorException {
        List<RequestAttribute> stored = attributeEngine
                .getRequestObjectDataAttributesContent(ObjectAttributeContentInfo
                        .builder(Resource.AUTHORITY, authority.getUuid())
                        .connector(authority.getConnectorUuid())
                        .build());
        return operationAttributeResolver.resolveForConnectorRequestAsSystem(authority.getConnectorUuid(), stored);
    }

    private List<RequestAttribute> resolvedRaProfileAttributes(RaProfile raProfile,
            AuthorityInstanceReference authority) throws ConnectorException {
        return operationAttributeResolver
                .resolveForConnectorRequestAsSystem(authority.getConnectorUuid(),
                        raProfileAttributesFor(raProfile, authority));
    }

    /**
     * Builds the body for the three v3 attribute-list endpoints (issue/revoke/register). Both attribute blobs are
     * required so the stateless connector can identify the upstream CA AND scope the returned schema to a specific RA
     * profile. {@code raProfile} may be null only for authority-wide listing flows that don't exist today — callers go
     * through the operator controller which always carries a profile.
     */
    private CertificateAttributeListRequestDtoV3 attributeListRequest(AuthorityInstanceReference authority,
            RaProfile raProfile) throws ConnectorException {
    ...
    private AdapterOperationResult mapIssueRenewRegisterResponse(ResponseEntity<CertificateDataResponseDto> response) {
        ...
        if (status == 200) {
            return AdapterOperationResult
                    .syncOk(body != null ? body.getCertificateData() : null, body != null ? body.getMeta() : null,
