...
public enum CertificateStateTransition {
    ...
    // Revoke failed or cancelled — restore.
    PENDING_REVOKE_TO_ISSUED(PENDING_REVOKE, ISSUED, CertificateEvent.REVOKE, CertificateEventStatus.FAILED),
    ...
    PENDING_APPROVAL_TO_PENDING_REVOKE(PENDING_APPROVAL, PENDING_REVOKE, CertificateEvent.APPROVAL_CLOSE,
            CertificateEventStatus.SUCCESS),
    // Revoke rejected — restore.
    PENDING_APPROVAL_TO_ISSUED(PENDING_APPROVAL, ISSUED, CertificateEvent.APPROVAL_CLOSE,
            CertificateEventStatus.FAILED),
    // Approved revoke completed synchronously (connector 200).
    PENDING_APPROVAL_TO_REVOKED(PENDING_APPROVAL, REVOKED, CertificateEvent.REVOKE, CertificateEventStatus.SUCCESS),
    ...
    // Issuing a registered placeholder requires approval.
    REGISTERED_TO_PENDING_APPROVAL(REGISTERED, PENDING_APPROVAL, CertificateEvent.APPROVAL_REQUEST,
            CertificateEventStatus.SUCCESS),
    // Issuing a registered placeholder failed.
    REGISTERED_TO_FAILED(REGISTERED, FAILED, CertificateEvent.ISSUE, CertificateEventStatus.FAILED),
    // Issuance approval rejected — restore the placeholder.
