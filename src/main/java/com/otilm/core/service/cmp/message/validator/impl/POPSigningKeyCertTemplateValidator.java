...
/**
 * <p>
 * POP validator if: The certificate subject places its name in the Certificate Template structure along with the public
 * key. In this case the poposkInput field is omitted from the POPOSigningKey structure. The signature field is computed
 * over the DER-encoded certificate template structure. See
 * <a href="https://www.rfc-editor.org/rfc/rfc4211#section-4.1">...</a>, point 3
 * </p>
 *
 * <pre>
...
public class POPSigningKeyCertTemplateValidator implements Validator<PKIMessage, Void> {
    ...
    public Void validate(PKIMessage message, ConfigurationContext configuration) throws CmpBaseException {
        ...
        if (subject == null) {
            throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP509);
        }
        ...
        try {
            ...
            if (!signature.verify(popoSigningKey.getSignature().getBytes())) {// podpisy nesedi
                throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP506);
            }
        } catch (SignatureException e) {
            throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP503, e);
        }
    ...
    /**
     * Try to get wrapper element {@link SubjectPublicKeyInfo} from {@link CertRequest} and validate its content (public
     * key bytes cannot be empty or zero).
     * <p>
     * if <code>poposkInput</code> is null, <code>public key</code> and subject must be filled (see POP/4.1 - point 3,
     * rfc 4211)
     *
     ...
     */
    private SubjectPublicKeyInfo getSubjectPublicKeyInfo(ASN1OctetString tid, CertRequest certRequest)
            throws CmpProcessingException {
        SubjectPublicKeyInfo subjectPublicKeyInfo = certRequest.getCertTemplate().getPublicKey();
        if (subjectPublicKeyInfo == null || subjectPublicKeyInfo.getPublicKeyData() == null
                || subjectPublicKeyInfo.getPublicKeyData().getBytes() == null
                || subjectPublicKeyInfo.getPublicKeyData().getBytes().length == 0) {
            throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP507);
        }
     ...
     */
    private PublicKey getPublicKey(ASN1OctetString tid, SubjectPublicKeyInfo subjectPublicKeyInfo)
            throws CmpProcessingException {
        try {
            return KeyFactory
                    .getInstance(subjectPublicKeyInfo.getAlgorithm().getAlgorithm().toString(),
            ...
            throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP501, e);
        }
    }
     ...
     */
    private Signature buildSignature(ASN1OctetString tid, CertReqMsg certReqMsg, PublicKey publicKey)
            throws CmpProcessingException {
        CertRequest certRequest = certReqMsg.getCertReq();
        ProofOfPossession proofOfPossession = certReqMsg.getPop();
        POPOSigningKey popoSigningKey = (POPOSigningKey) proofOfPossession.getObject();
        try {
             ...
             * AlgorithmIdentifier, signature BIT STRING } -- The signature (using "algorithmIdentifier") is on the --
             * DER-encoded value of poposkInput. NOTE: If the CertReqMsg -- certReq CertTemplate contains the subject
             * and publicKey values, -- then poposkInput MUST be omitted and the signature MUST be -- computed over the
             * DER-encoded value of CertReqMsg certReq. If -- the CertReqMsg certReq CertTemplate does not contain both
             * the -- public key and subject values (i.e., if it contains only one -- of these, or neither), then
             * poposkInput MUST be present and -- MUST be signed.
             *
             * @see https://www.rfc-editor.org/rfc/rfc4211#appendix-B
             *
             * The certificate subject places its name in the Certificate Template structure along with the public key.
             * In this case the poposkInput field is omitted from the POPOSigningKey structure. The signature field is
             * computed over the DER-encoded certificate template structure.
             *
             * @see https://www.rfc-editor.org/rfc/rfc4211#section-4.1 (point 3)
             */
            byte[] subjectOfVerification = certRequest.getEncoded(ASN1Encoding.DER);
            Signature signature = Signature
                    .getInstance(popoSigningKey.getAlgorithmIdentifier().getAlgorithm().getId(),
                            BouncyCastleProvider.PROVIDER_NAME);
            ...
            return signature;
                ...
                | IOException e) {
            throw new CmpProcessingException(tid, PKIFailureInfo.badPOP, ImplFailureInfo.CMPVALPOP502, e);
        }
