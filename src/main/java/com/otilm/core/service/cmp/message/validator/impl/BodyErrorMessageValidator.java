...
/**
 * Validator of {@link PKIMessage} which contains {@link ErrorMsgContent} which is subject for validation.
 */
public class BodyErrorMessageValidator extends BaseValidator implements Validator<PKIMessage, Void> {
     ...
     *
     * <p>
     * This message MAY be generated at any time during a PKI transaction. If the client sends this request, the server
     * MUST respond with a PKIConfirm response, or another ErrorMsg if any part of the header is not valid. Both sides
     * MUST treat this message as the end of the transaction (if a transaction is in progress).
     * </p>
     *
     * <p>
     * If protection is desired on the message, the client MUST protect it using the same technique (i.e., signature or
     * MAC) as the starting message of the transaction. The CA MUST always sign it with a signature key.
     * </p>
     *
    ...
    public Void validate(PKIMessage response, ConfigurationContext configuration) throws CmpBaseException {
        ...
        checkValueNotNull(response.getHeader().getTransactionID(), pkiStatusInfo, PKIFailureInfo.badDataFormat,
                "PKIStatusInfo");
