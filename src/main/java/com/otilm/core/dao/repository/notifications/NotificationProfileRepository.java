...
public interface NotificationProfileRepository extends SecurityFilterRepository<NotificationProfile, UUID> {
    ...
    /**
     * Pessimistic-write variant of {@code findByUuid} for paths that assign the next profile version number. Issues
     * {@code SELECT ... FOR UPDATE} on the profile row so concurrent edits serialize instead of both reading the same
     * latest version and inserting duplicate version numbers. Must be called inside an active transaction, otherwise
     * the lock is released immediately on query completion.
     */
