...
public class UserManagementApiClient extends PlatformBaseAuthenticationClient {
    ...
    public UserWithPaginationDto getUsers() {
        ...
        return processRequest(r -> r.retrieve().toEntity(UserWithPaginationDto.class).block().getBody(), request);
    }

    public UserDetailDto getUserDetail(String userUuid) {
        WebClient.RequestBodySpec request = prepareRequest(HttpMethod.GET)
                .uri(USER_DETAIL_CONTEXT.replace("{userUuid}", userUuid));

        return processRequest(r -> r.retrieve().toEntity(UserDetailDto.class).block().getBody(), request);
    }
    ...
    public void removeUser(String userUuid) {
        ...
        processRequest(r -> r.uri(USER_DETAIL_CONTEXT, userUuid).retrieve().toEntity(Void.class).block().getBody(),
                request);
    }
    ...
    public List<RoleDto> getUserRoles(String userUuid) {
        ...
        return processRequest(
                r -> r.uri(USER_ROLE_CONTEXT, userUuid).retrieve().toEntityList(RoleDto.class).block().getBody(),
                request);
    }
